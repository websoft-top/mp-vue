...
/* 搜索 */
const reload = (where?: HjmFenceParam) => {
  selection.value = [];
...
/* 删除单个 */
const remove = (row: HjmFence) => {
  const hide = message.loading('请求中..', 0);
  removeHjmFence(row.id)
    .then((msg) => {
      hide();
      message.success(msg);
      reload();
...
const removeBatch = () => {
  ...
  Modal.confirm({
    ...
    onOk: () => {
      const hide = message.loading('请求中..', 0);
      removeBatchHjmFence(selection.value.map((d) => d.id))
        .then((msg) => {
          hide();
          message.success(msg);
          reload();
...
/* 自定义行属性 */
const customRow = (record: HjmFence) => {
  return {
    // 行点击事件
    onClick: () => {
      // console.log(record);
